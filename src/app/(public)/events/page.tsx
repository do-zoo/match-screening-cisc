...
export default async function PublicEventsPage() {
  ...
  return (
    <main className='mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-4 md:px-6 py-12'>
      <div>
        <h1 className='font-semibold text-2xl tracking-tight md:text-3xl'>Acara aktif</h1>
        <p className='mt-2 max-w-2xl text-sm text-[hsl(var(--muted-foreground))]'>
          Daftar nobar dan kegiatan yang sedang dibuka pendaftarannya. Pilih kartu untuk membuka formulir.
        </p>
      </div>

      {events.length === 0 ? (
        <p className='text-sm text-[hsl(var(--muted-foreground))]'>Belum ada acara aktif.</p>
      ) : (
