    ...
    <main className='mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-4 md:px-6 py-8 lg:py-10'>
      <header className='flex flex-col gap-2'>
        ...
        <p className='text-sm text-muted-foreground'>
          Pratinjau backoffice <span className='font-medium text-foreground'>{branding.clubNameNav}</span>— anggota,
          acara, venue, dan kepengurusan dalam satu tempat.
        </p>
      </header>
      ...
      <Alert className='border-primary/40 bg-muted/40'>
        ...
        <AlertDescription className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
          <span>{fmtNum.format(pendingTotal)} registrasi berstatus menunggu tindakan di acara yang Anda akses.</span>
          <Link
            href='/admin/events?tab=active'
            ...
            .map(({ href, title, description, Icon }) => (
              <li key={href}>
                <Link href={href} className='block h-full rounded-lg focus-visible:outline-none'>
                  <Card className={cn('h-full transition-colors hover:border-primary/30 hover:bg-muted/30')}>
