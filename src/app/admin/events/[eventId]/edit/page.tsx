...
export default async function AdminEditEventPage({
  ...
  const sortedEventLinks = [...event.eventVenueMenuItems].sort(
    (a, b) => (a.sortOrder ?? a.venueMenuItem.sortOrder) - (b.sortOrder ?? b.venueMenuItem.sortOrder),
  )
  ...
  const linkedVenueMenuItems =
    event._count.registrations === 0 && fullCatalogLinked.length > 0 ? fullCatalogLinked : persistedLinkedDraft
    ...
    <main className='mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 md:p-6 px-4 md:px-6 pb-16 pt-8 lg:pt-6'>
      <header className='flex flex-col gap-2'>
        ...
        {event._count.registrations > 0 ? (
          <p className='text-amber-800 text-sm dark:text-amber-200'>Ada {event._count.registrations} registrasi</p>
        ) : null}
      </header>
