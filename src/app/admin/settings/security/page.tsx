...
export default async function SecuritySettingsPage({
  ...
  const actionOptions = Object.values(CLUB_AUDIT_ACTION).sort((a, b) => a.localeCompare(b))
    ...
    <div className='space-y-10'>
      ...
      <section className='space-y-3 rounded-lg border bg-card p-4'>
        ...
        <p className='text-muted-foreground text-sm leading-relaxed'>
          ...
          . Verifikasi tambahan dengan kode email pada saat masuk hanya aktif jika pengiriman email transaksional telah
          dikonfigurasi di lingkungan server. Dokumentasi Better Auth:{' '}
          <a
      ...
      <section className='space-y-3'>
        ...
        <p className='text-muted-foreground text-sm'>
          Paginasi 10 entri per halaman. Filter waktu memakai tanggal dan jam lokal (kalender + field waktu); batas
          filter disimpan sebagai momen absolut (ISO) dan dibandingkan ke stempel waktu di basis data. Metadata
          disanitasi; tidak berisi isi penuh templat WhatsApp.
        </p>
        <ClubAuditLogSection
          rows={rows}
