...
import { listPeriodRolesAsTree, type PeriodTreeRow } from '@/lib/management/query-admin-period-tree'
          ...
          return (
            <View key={row.roleId} style={styles.row}>
              ...
              <View style={styles.assigneeCell}>
                <Text style={row.assignees.length === 0 ? styles.emptyAssignee : undefined}>{assigneeText}</Text>
              </View>
            </View>
          )
        })}
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ periodId: string }> }) {
  ...
  const buffer = await renderToBuffer(<OrgDocument periodLabel={period.label} dateRange={dateRange} rows={rows} />)
