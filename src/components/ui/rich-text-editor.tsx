  ...
  const handleImageFile = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      startImgTransition(async () => {
        ...
        const res = await uploadEventDescriptionImage(
          descriptionImageUpload.eventId,
          undefined,
          fd,
        );
        if (!res.ok) {
  ...
  const apply = useCallback(() => {
    ...
    if (trimmed === "") {
      ...
      editor
        ...
        .setLink({ href: withScheme, target: "_blank" })
        .run();
    }
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="w-80" align="start">
        <PopoverHeader>
          ...
          <PopoverDescription>
            Tempel URL lengkap (https://…) atau alamat surel.
          </PopoverDescription>
        </PopoverHeader>
