...
import { COUNTRIES, type CountryEntry } from '@/components/ui/phone-input-countries'
...
export function PhoneInput({
  ...
  const selected: CountryEntry = COUNTRIES.find(c => c.iso === value?.countryIso) ?? DEFAULT_COUNTRY
  ...
  return (
    <InputGroup>
      <InputGroupAddon align='inline-start' className='shrink-0 self-stretch p-0 pl-0 has-[>button]:ml-0'>
        <Popover open={open} onOpenChange={setOpen}>
          ...
          >
            ...
            <ChevronDownIcon className='size-3.5 opacity-55' aria-hidden='true' />
          </PopoverTrigger>
          <PopoverContent align='start' side='bottom' className='w-64 p-0'>
            ...
            <div ref={listRef} role='listbox' aria-label='Pilih negara' className='max-h-60 overflow-y-auto'>
              {filtered.length === 0 ? (
                <div className='px-3 py-6 text-center text-sm text-muted-foreground'>Negara tidak ditemukan</div>
              ) : (
