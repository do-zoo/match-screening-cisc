...
import { createMasterMember, updateMasterMember } from '@/lib/actions/admin-master-members'
...
export function MemberFormDialog({ mode, open, onOpenChange, member, onSaved }: Props) {
  ...
  const form = useForm<MemberFormValues>({
    resolver: zodResolver(
      (mode === 'create' ? adminMasterMemberCreateSchema : adminMasterMemberUpdateSchema) as never,
  ...
  function submit(values: MemberFormValues) {
    ...
    startTransition(async () => {
      ...
      const result =
        mode === 'create' ? await createMasterMember(undefined, fd) : await updateMasterMember(undefined, fd)
      ...
      if (!result.ok) {
        ...
        setRootMessage(
          result.rootError ??
            (Object.keys(fieldErrors).length > 0 ? 'Periksa kembali isian anggota.' : 'Terjadi kesalahan.'),
        )
        return
      }
      ...
      toastCudSuccess(
        ...
        mode === 'create' ? 'Anggota berhasil ditambahkan.' : 'Anggota berhasil diperbarui.',
      )
      onOpenChange(false)
      onSaved()
    })
  ...
  const description =
    mode === 'create' ? 'Tambahkan data anggota baru ke master anggota.' : 'Perbarui data anggota di master anggota.'
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        ...
        <form className='flex flex-col gap-4' onSubmit={form.handleSubmit(submit)}>
          ...
          <Field label='Nomor member' htmlFor='member-number' error={form.formState.errors.memberNumber?.message}>
            <Input
              id='member-number'
          ...
          <Field label='Nama' htmlFor='member-full-name' error={form.formState.errors.fullName?.message}>
            <Input
              id='member-full-name'
              disabled={isPending}
          ...
          <div className='grid gap-3 rounded-lg border p-3'>
            <BooleanField control={form.control} name='isActive' label='Aktif' disabled={isPending} />
          </div>
  ...
  return (
    <Controller
      ...
      render={({ field }) => (
        <label
          htmlFor={id}
          className={cn('flex cursor-pointer items-center gap-2 text-sm', disabled && 'cursor-not-allowed opacity-60')}
        >
          <Checkbox
            id={id}
