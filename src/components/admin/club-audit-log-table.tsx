...
export function ClubAuditLogFilters(props: {
  ...
  return (
    <form method='get' action={props.securityPath} className='flex flex-col gap-4 rounded-lg border bg-card/30 p-4'>
      ...
      <div className='flex flex-col gap-3'>
        <div className='grid gap-3 sm:grid-cols-2'>
          ...
          />
          <ClubAuditLogDateTimeField id='audit-to' name='to' label='Sampai tanggal & waktu' defaultValue={props.to} />
        </div>
        <div className='grid gap-3 sm:grid-cols-2'>
          <div className='space-y-1.5'>
            <Label htmlFor='audit-action'>Aksi</Label>
            <ClubAuditLogActionSelect id='audit-action' defaultAction={props.action} options={props.actionOptions} />
          </div>
          <div className='space-y-1.5'>
      ...
      <div className='flex flex-wrap items-center gap-2'>
        ...
        <Link href={props.securityPath} className={buttonVariants({ variant: 'ghost', size: 'sm' })}>
          Reset
        </Link>
      </div>
    </form>
...
export function ClubAuditLogTable(props: { rows: ClubAuditRowVm[]; emptyMessage?: string }) {
  if (props.rows.length === 0) {
    return (
      <p className='text-muted-foreground text-sm'>{props.emptyMessage ?? 'Belum ada peristiwa audit tercatat.'}</p>
    )
  }
...
export function ClubAuditLogSection(props: {
  ...
  const hasActiveFilters = Boolean(
    props.filters.from || props.filters.to || props.filters.action || props.filters.userId,
  )
  ...
  return (
    <div className='space-y-4'>
      ...
      <ClubAuditLogTable
        rows={props.rows}
        emptyMessage={hasActiveFilters ? 'Tidak ada entri audit yang cocok dengan filter.' : undefined}
      />
      <TablePagination
