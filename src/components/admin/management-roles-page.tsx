...
function buildManagementRolesHref(opts: { filter: BoardRoleAdminFilter; q: string }): string {
    ...
    () => [
      {
        accessorKey: 'title',
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title='Nama Jabatan' className='text-muted-foreground' />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.isUnique ? (
            <Badge className='bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400'>1 orang</Badge>
          ) : (
            <Badge className='bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400'>Banyak</Badge>
          ),
      },
      {
        accessorKey: 'sortOrder',
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title='Urutan' className='text-muted-foreground' />
        ),
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title='Status' className='text-muted-foreground' />
        ),
        cell: ({ row }) =>
          ...
          <div className='text-right'>
            <DropdownMenu>
              ...
              <DropdownMenuContent align='end'>
                <DropdownMenuItem onClick={() => setEditDialog({ role: row.original, mode: 'edit' })}>
                  Edit jabatan
                </DropdownMenuItem>
                {row.original.isActive ? (
                  <>
        ...
        <div className='flex flex-col gap-4'>
          ...
          >
            ...
            <div className='flex w-full flex-col gap-1 sm:w-auto sm:min-w-52'>
              <Label htmlFor='management-roles-filter' className='text-muted-foreground'>
                Filter
              </Label>
              <Select
                value={filter}
                onValueChange={next => {
                  ...
                  router.push(buildManagementRolesHref({ filter: next, q: searchQuery }))
                }}
              >
                <SelectTrigger id='management-roles-filter' size='sm' className='w-full'>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
