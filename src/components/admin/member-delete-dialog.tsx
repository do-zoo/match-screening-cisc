...
export function MemberDeleteDialog({ member, open, onOpenChange, onDeleted }: Props) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className='sm:max-w-md'>
        <DialogHeader>
          ...
          <DialogDescription>
            Hapus <strong>{name}</strong> secara permanen? Tindakan ini tidak bisa dibatalkan.
          </DialogDescription>
        </DialogHeader>
        {error ? (
        ...
        <DialogFooter>
          ...
          <Button type='button' variant='destructive' disabled={isPending || !member} onClick={handleConfirm}>
            {isPending ? <Loader2 className='size-4 animate-spin' /> : 'Ya, hapus anggota'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
