...
export function AdminPicBankAccountsInline(props: AdminPicBankAccountsInlineProps) {
  ...
  const canMutate = useMemo(
    () => canMutatePicBankForTarget(props.viewerRole, props.viewerProfileId, props.ownerAdminProfileId),
    [props.viewerRole, props.viewerProfileId, props.ownerAdminProfileId],
  )

  const [addOpen, setAddOpen] = useState(false)
  const [editAccount, setEditAccount] = useState<CommitteeAdminDirectoryPicBankVm | null>(null)
  const [deactivateAccount, setDeactivateAccount] = useState<CommitteeAdminDirectoryPicBankVm | null>(null)
  const [deleteAccount, setDeleteAccount] = useState<CommitteeAdminDirectoryPicBankVm | null>(null)
  ...
  const [deactivateState, deactivateDispatch, deactivatePending] = useActionState(
    deactivatePicBankAccount,
    null as ActionResult<{ saved: true }> | null,
  )
  ...
  return (
    <div className='space-y-4 py-4'>
      <div className='flex flex-wrap items-center justify-between gap-2'>
        ...
        {canMutate ? (
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            <DialogTrigger render={<Button variant='outline' size='sm' />}>Tambah rekening…</DialogTrigger>
            <DialogContent className='sm:max-w-md'>
              ...
              <form action={createDispatch} className='space-y-3' key={`pb-add-${props.ownerAdminProfileId}`}>
                ...
                <DialogFooter>
                  <Button type='submit' disabled={createPending}>
                    {createPending ? <Loader2 className='size-4 animate-spin' /> : 'Simpan rekening'}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
      ...
      {props.accounts.length === 0 ? (
        <p className='text-muted-foreground text-sm'>Belum ada rekening PIC untuk profil ini.</p>
      ) : (
            ...
            <li key={a.id} className='flex flex-wrap items-start justify-between gap-3 px-3 py-3'>
              ...
              {canMutate ? (
                <div className='flex flex-wrap gap-2'>
                  <>
                    <Button variant='outline' size='sm' type='button' onClick={() => setEditAccount(a)}>
                      Ubah…
                    </Button>
                    <Dialog
                      ...
                      >
                        <DialogContent className='sm:max-w-md'>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Acara yang sudah menggunakan rekening ini tetap terpasang. Acara baru harus memilih
                              rekening aktif lain bagi PIC tersebut.
                            </DialogDescription>
                          </DialogHeader>
                          <form action={deactivateDispatch} className='space-y-3'>
                            ...
                            <DialogFooter>
                              <Button type='submit' variant='destructive' disabled={deactivatePending}>
                                {deactivatePending ? <Loader2 className='size-4 animate-spin' /> : 'Nonaktifkan'}
                              </Button>
                            </DialogFooter>
                          </form>
                        </DialogContent>
                    ...
                    >
                      <DialogContent className='sm:max-w-md'>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Hanya diperbolehkan jika tidak ada acara yang memakai rekening ini. Jika masih dipakai,
                            gunakan nonaktif atau ganti rekening di acara.
                          </DialogDescription>
                        </DialogHeader>
                        <form action={deleteDispatch} className='space-y-3'>
                          ...
                          <DialogFooter>
                            <Button type='submit' variant='destructive' disabled={deletePending}>
                              {deletePending ? <Loader2 className='size-4 animate-spin' /> : 'Hapus permanen'}
                            </Button>
                          </DialogFooter>
                        </form>
                      </DialogContent>
