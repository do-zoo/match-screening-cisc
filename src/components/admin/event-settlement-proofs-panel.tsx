...
} from "@/lib/reports/settlement-expected-amounts";
import { formatIdr } from "@/lib/utils/format-idr";

const KIND_LABEL: Record<EventSettlementArtifactKind, string> = {
  [EventSettlementArtifactKind.venue_transfer]:
    "Bukti transfer ke venue",
  [EventSettlementArtifactKind.venue_receipt]:
    "Nota / bukti terima dari venue",
  [EventSettlementArtifactKind.treasurer_margin]:
    "Bukti transfer margin ke bendahara",
};
...
export function EventSettlementProofsPanel({
  ...
  function handleRequestUpload(
    ...
    if (
      kind !== EventSettlementArtifactKind.venue_receipt &&
  ...
  function confirmMismatchUpload() {
    ...
    if (!mismatchAck || mismatchReason.trim().length < 3) {
      toast.error(
        "Centang konfirmasi dan isi alasan selisih (minimal 3 karakter).",
      );
      return;
    }
    runUpload(
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Unggahan bersifat riwayat (beberapa versi). PIC acara atau
            Owner/Admin dapat menambah bukti. Nominal transfer ke venue dan ke
            bendahara dibandingkan dengan acuan laporan; selisih di luar{" "}
            {formatIdr(SETTLEMENT_AMOUNT_TOLERANCE_IDR)} wajib konfirmasi +
            alasan.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-8">
          {canManage ? (
            <>
              <section className="space-y-3 rounded-lg border p-4">
                <h3 className="text-sm font-medium">
                  {KIND_LABEL[EventSettlementArtifactKind.venue_transfer]}
                </h3>
                <p className="text-muted-foreground text-xs">
                  Acuan nominal: {formatIdr(expectedVenueMenuPayout)} (agregat
                  menu wajib approved)
                </p>
              ...
              <section className="space-y-3 rounded-lg border p-4">
                <h3 className="text-sm font-medium">
                  {KIND_LABEL[EventSettlementArtifactKind.venue_receipt]}
                </h3>
                <p className="text-muted-foreground text-xs">
                  Tidak memerlukan nominal; unggah nota atau bukti terima dari
                  venue.
                </p>
                <div className="flex max-w-lg flex-col gap-3 sm:flex-row sm:items-end">
                  <div className="min-w-0 flex-1">
              ...
              <section className="space-y-3 rounded-lg border p-4">
                <h3 className="text-sm font-medium">
                  {KIND_LABEL[EventSettlementArtifactKind.treasurer_margin]}
                </h3>
                <p className="text-muted-foreground text-xs">
                  Acuan nominal: {formatIdr(expectedTreasurerMargin)} (tiket
                  approved + penyesuaian lunas)
                </p>
                <div className="max-w-xs space-y-2">
            ...
            <p className="text-muted-foreground text-sm">
              Hanya PIC acara atau Owner/Admin yang dapat menambah bukti baru.
              Riwayat di bawah tetap dapat dibaca.
            </p>
          )}
                    ...
                    {artifacts.map((a) => (
                      <tr key={a.id} className="border-b last:border-0">
                        ...
                        <td className="p-2 font-mono tabular-nums">
                          {a.declaredAmountIdr == null
                            ? "—"
                            : formatIdr(a.declaredAmountIdr)}
                        </td>
                        <td className="p-2 font-mono tabular-nums text-xs">
                          {a.amountDeltaIdr == null
                            ? "—"
                            : formatIdr(a.amountDeltaIdr)}
                        </td>
      ...
      <Dialog open={mismatchOpen} onOpenChange={setMismatchOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Nominal yang Anda masukkan berbeda lebih dari{" "}
              {formatIdr(SETTLEMENT_AMOUNT_TOLERANCE_IDR)} dari angka acuan.
              Konfirmasi dan jelaskan agar bukti tetap dapat disimpan.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="flex items-center gap-2">
              <Checkbox
                checked={mismatchAck}
                onCheckedChange={(v) => setMismatchAck(v === true)}
              />
              <Label className="text-sm font-normal">
                Saya mengonfirmasi selisih ini disengaja dan alasan di bawah
                benar.
              </Label>
            </div>
