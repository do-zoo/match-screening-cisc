...
import type { AdminEventSummary, EventsIndexStatusTab } from '@/lib/admin/events-index-view-model'
    ...
    <div className='flex flex-col gap-8'>
      {events.length === 0 ? (
        <div className='rounded-lg border border-dashed bg-card p-8 text-center text-sm text-muted-foreground'>
          Tidak ada acara untuk ditampilkan dengan filter ini. Jika Anda Viewer, Anda hanya melihat acara sebagai helper
          — pastikan Anda ditugaskan.
        </div>
      ) : (
        <>
          <ul className='grid list-none gap-4 p-0 sm:grid-cols-2 xl:grid-cols-3'>
            {events.map(item => (
              <EventSummaryCard key={item.id} card={item} showSettingsLink={showEventSettingsLink} />
            ))}
          </ul>
          <TablePagination
...
function EventSummaryCard({ card, showSettingsLink }: { card: AdminEventSummary; showSettingsLink: boolean }) {
    ...
    <li>
      <Card className='flex h-full flex-col'>
        ...
        <CardFooter className='mt-auto flex flex-wrap gap-3 border-t pt-4'>
          <Link href={eventRegistrantsListPath(card.id)} className={buttonVariants({ className: 'inline-flex' })}>
            Buka peserta
          </Link>
          <Link
