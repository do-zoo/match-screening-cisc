...
import {
  venueMenuRowMatchesLockFilter,
  venueMenuRowMatchesSearch,
} from "@/lib/admin/filter-venue-menu-list";
...
function mapInitialToMenuRows(
  source: VenueCatalogUiPayload["items"],
): MenuRow[] {
...
export function AdminVenueMenuPanel({
  ...
  const [items, setItems] = React.useState<MenuRow[]>(() =>
    mapInitialToMenuRows(initialItems),
  );
  const [imageFiles, setImageFiles] = React.useState<Record<string, File>>({});

  const [itemDialogOpen, setItemDialogOpen] = React.useState(false);
  const [itemDialogMode, setItemDialogMode] = React.useState<"create" | "edit">(
    "create",
  );
  ...
  const frozenMenuItemIdSet = React.useMemo(
    () => new Set(frozenMenuItemIds),
    [frozenMenuItemIds],
  );
  ...
  const displayPage = resolveClampedPage(
    listQuery.page,
    totalFiltered,
    ADMIN_TABLE_PAGE_SIZE,
  );
  ...
  const menuSignature = React.useMemo(
    () => serverMenuSignature(initialItems),
    [initialItems],
  );
  ...
  const deleteTarget =
    deleteIndex !== null ? (items[deleteIndex] ?? null) : null;
  const deleteFrozen =
    deleteTarget?.id != null && frozenMenuItemIdSet.has(deleteTarget.id);
    ...
    <div className="flex flex-col gap-4 md:p-6">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          Item menu dipakai saat menyusun acara di venue ini. Ubah nama atau
          harga pada item yang sudah dipakai pendaftar akan ditolak.
        </p>
      </header>
      ...
      <div className="space-y-4">
        <AdminListToolbar
          ...
          filterSlot={
            <>
              <Label
                htmlFor="admin-venue-menu-filter"
        ...
        {totalFiltered === 0 ? (
          <p className="text-muted-foreground text-sm">
            Tidak ada item yang cocok dengan pencarian atau filter. Ubah filter,
            hapus teks cari, atau tambah item baru.
          </p>
        ) : null}
            ...
            <Table>
              ...
              <TableBody>
                {pagedIndexedRows.map(({ row, index }) => {
                  ...
                  const frozen = row.id
                    ? frozenMenuItemIdSet.has(row.id)
                    : false;
                  return (
                    <TableRow key={key}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {pagedIndexedRows.map(({ row, index }) => {
                ...
                return (
                  <div
                    key={key}
                    className="bg-card flex flex-col gap-3 rounded-lg border p-4 shadow-xs"
                  >
            ...
            <div className="grid gap-4 py-1">
              ...
              <div className="grid gap-2">
                <FileField
                  ...
                  disabled={pending}
                  existingPreviewUrl={
                    draft && !draft.clearImage
                      ? (draft.imageBlobUrl ?? undefined)
                      : undefined
                  }
        ...
        <DialogContent className="sm:max-w-md" showCloseButton={!pending}>
            ...
            <DialogDescription>
              {deleteTarget ? (
                <>
                  ...
                  akan dihapus dari katalog venue. Tindakan ini tidak bisa
          ...
          {deleteFrozen ? (
            <p className="text-destructive text-sm">
              Item ini tidak bisa dihapus karena sudah dipakai acara dengan
              pendaftaran.
            </p>
          ) : null}
          <DialogFooter>
