...
import type { AdminDashboardEventCard, DashboardEventTab } from "@/lib/admin/dashboard-view-model";
...
export function AdminEventsDashboardCards({
  session,
  tab,
  events,
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-6 py-8 lg:py-10">
      <header className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
        ...
        {showTableViewLink || showCreateEventButton ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            {showTableViewLink ? (
              <Link
                href="/admin/events?view=tabel"
                className={buttonVariants({ variant: "outline", size: "sm", className: "shrink-0" })}
              >
                Tampilan tabel (paginasi)
              </Link>
            ) : null}
          </div>
        ) : null}
      ...
      <div className="flex flex-wrap gap-2" role="tablist" aria-label="Filter status acara">
        {tabDefs.map(({ tab: key, label }) => {
          const href = `/admin/events?tab=${key}`;
