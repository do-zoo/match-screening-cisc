...
import { DownloadIcon, MoreVerticalIcon, PlusIcon, SearchIcon } from 'lucide-react'
  ...
  const columns = useMemo<ColumnDef<AdminMasterMemberRowVm>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <BooleanBadge value={row.original.isManagementMember} trueLabel='Ya' falseLabel='Tidak' />,
      },
      {
        ...
        cell: ({ row }) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align='end'>
              <DropdownMenuItem onClick={() => setEditingMember(row.original)}>Edit anggota</DropdownMenuItem>
              {isOwner ? (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem variant='destructive' onClick={() => setMemberPendingDelete(row.original)}>
                    Hapus anggota
                  </DropdownMenuItem>
                </>
              ) : null}
    ...
    <main className='mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 md:p-6 px-4 md:px-6 py-8 lg:py-10'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Basis data dipaginasikan dari server ({pagination.totalItems} hasil untuk filter ini).
          </CardDescription>
        </CardHeader>
