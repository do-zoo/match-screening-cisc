  ...
  return (
    <main className='mx-auto flex w-full max-w-6xl flex-1 flex-col gap-14 px-4 md:px-6 py-12 md:gap-16 md:py-16'>
      <FadeContent blur className='max-w-2xl' threshold={0.15} duration={900}>
        <p className='text-xs font-medium tracking-widest text-[hsl(var(--muted-foreground))] uppercase'>Komunitas</p>
        ...
        <p className='mt-4 text-pretty text-base leading-relaxed text-[hsl(var(--muted-foreground))] md:text-lg'>
          Daftar untuk nobar dan acara resmi klub. Pilih acara pada daftar, lengkapi formulir, unggah bukti transfer,
          lalu tim verifikator akan memproses pendaftaran Anda.
        </p>
        <div className='mt-8 flex flex-wrap gap-3'>
          <Link href='/events' className={cn(buttonVariants({ variant: 'default', size: 'lg' }))}>
            Lihat semua acara
          </Link>
          <a
            href='#alur-pendaftaran'
            className={cn(buttonVariants({ variant: 'outline', size: 'lg' }), 'text-[hsl(var(--foreground))]')}
          >
            Alur pendaftaran
          </a>
        </div>
        ...
        >
          <li>
            <span className='font-medium text-[hsl(var(--foreground))]'>Pilih acara</span> pada halaman daftar.
          </li>
          <li>
            <span className='font-medium text-[hsl(var(--foreground))]'>Isi formulir</span> dan lampirkan bukti
            pembayaran.
          </li>
          <li>
            <span className='font-medium text-[hsl(var(--foreground))]'>Tunggu verifikasi</span> dari tim penyelenggara.
          </li>
        </ol>
      ...
      {previewEvents.length > 0 ? (
        <FadeContent className='flex flex-col gap-5' threshold={0.12} delay={0.15}>
          <div className='flex flex-wrap items-end justify-between gap-3'>
            <h2 className='font-semibold text-xl tracking-tight md:text-2xl'>Acara mendatang</h2>
