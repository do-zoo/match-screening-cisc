...
export function StepIndicator({ current }: Props) {
  return (
    <nav aria-label='Langkah pendaftaran' className='mb-6 flex items-start justify-evenly gap-0'>
      <StepBubble n={1} label='Data Peserta' state={current === 1 ? 'active' : 'done'} />
      <div className={cn('mt-4 h-0.5 flex-1 shrink-0 transition-colors', current > 1 ? 'bg-primary' : 'bg-border')} />
      <StepBubble n={2} label='Ringkasan' state={current === 2 ? 'active' : 'upcoming'} />
    </nav>
  )
}
