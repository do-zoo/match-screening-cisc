...
function masterMemberIdOrNull(v: string | null | undefined): string | null {
...
export async function createManagementMember(
  ...
  const parsed = adminManagementMemberCreateSchema.safeParse(parseJsonPayload(formData))
  ...
  const masterMemberId = masterMemberIdOrNull(parsed.data.masterMemberId ?? null)

  const whatsappStored =
    parsed.data.whatsapp && parsed.data.whatsapp.trim().length > 0 ? parsed.data.whatsapp.trim() : null
  ...
  try {
    const row = await prisma.$transaction(async tx => {
      ...
      await recomputeDirectoryManagementFlagsTx(tx, created.masterMemberId ? [created.masterMemberId] : [])
      return created
    })
...
export async function updateManagementMember(
  ...
  const parsed = adminManagementMemberUpdateSchema.safeParse(parseJsonPayload(formData))
  if (!parsed.success) return { ok: false, fieldErrors: zodToFieldErrors(parsed.error) }
  ...
  try {
    await prisma.$transaction(async tx => {
      ...
      const seeds = [before.masterMemberId, masterMemberId].filter((x): x is string => Boolean(x))
...
export async function deleteManagementMember(
  ...
  const parsed = deleteManagementMemberSchema.safeParse(parseJsonPayload(formData))
  if (!parsed.success) return { ok: false, fieldErrors: zodToFieldErrors(parsed.error) }
  ...
  try {
    await prisma.$transaction(async tx => {
      ...
      await recomputeDirectoryManagementFlagsTx(tx, before.masterMemberId ? [before.masterMemberId] : [])
    })
  } catch (e) {
    console.error(e)
