...
async function seedIdsFromManagementMemberIds(tx: Prisma.TransactionClient, ids: string[]): Promise<string[]> {
  ...
  return rows.map(r => r.masterMemberId).filter((id): id is string => id !== null)
}

export async function createBoardAssignment(_prev: unknown, formData: FormData): Promise<ActionResult<{ id: string }>> {
  try {
  ...
  const parsed = adminBoardAssignmentUpsertSchema.safeParse(parseJsonPayload(formData))
  ...
  try {
    const row = await prisma.$transaction(async tx => {
      ...
      const seeds = await seedIdsFromManagementMemberIds(tx, [parsed.data.managementMemberId])
...
export async function updateBoardAssignment(_prev: unknown, formData: FormData): Promise<ActionResult<{ id: string }>> {
  try {
    await guardOwnerOrAdmin()
  } catch (e) {
    if (isAuthError(e)) return rootError('Tidak diizinkan.')
  ...
  const parsed = adminBoardAssignmentUpdateSchema.safeParse(parseJsonPayload(formData))
  if (!parsed.success) return { ok: false, fieldErrors: zodToFieldErrors(parsed.error) }
...
export async function deleteBoardAssignment(
  ...
  const parsed = deleteBoardAssignmentSchema.safeParse(parseJsonPayload(formData))
  if (!parsed.success) return { ok: false, fieldErrors: zodToFieldErrors(parsed.error) }
  ...
  try {
    await prisma.$transaction(async tx => {
      ...
      const seeds = await seedIdsFromManagementMemberIds(tx, [before.managementMemberId])
      await recomputeDirectoryManagementFlagsTx(tx, seeds)
    })
  } catch (e) {
    if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2025') {
