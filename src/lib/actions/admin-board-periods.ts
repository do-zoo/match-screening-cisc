...
async function overlapExists(candidate: { startsAt: Date; endsAt: Date }, excludeId?: string): Promise<boolean> {
  const rows = await prisma.boardPeriod.findMany({
    select: { id: true, startsAt: true, endsAt: true },
  })
  return rows.some(row => row.id !== excludeId && periodsOverlap(candidate, row))
}

export async function createBoardPeriod(_prev: unknown, formData: FormData): Promise<ActionResult<{ id: string }>> {
  try {
  ...
  const parsed = adminBoardPeriodCreateSchema.safeParse(parseJsonPayload(formData))
...
export async function updateBoardPeriod(_prev: unknown, formData: FormData): Promise<ActionResult<{ id: string }>> {
  try {
    await guardOwnerOrAdmin()
  } catch (e) {
    if (isAuthError(e)) return rootError('Tidak diizinkan.')
  ...
  const parsed = adminBoardPeriodUpdateSchema.safeParse(parseJsonPayload(formData))
  if (!parsed.success) return { ok: false, fieldErrors: zodToFieldErrors(parsed.error) }
...
export async function deleteBoardPeriod(_prev: unknown, formData: FormData): Promise<ActionResult<{ deleted: true }>> {
  try {
    await guardOwnerOrAdmin()
  } catch (e) {
    if (isAuthError(e)) return rootError('Tidak diizinkan.')
  ...
  const parsed = deleteBoardPeriodSchema.safeParse(parseJsonPayload(formData))
  if (!parsed.success) return { ok: false, fieldErrors: zodToFieldErrors(parsed.error) }
