...
export async function lookupManagementCodeForRegistration(raw: string): Promise<LookupManagementCodeResult> {
  ...
  if (!r.ok) {
    return r.reason === 'not_found' ? { kind: 'not_found' } : { kind: 'not_assigned' }
  }
