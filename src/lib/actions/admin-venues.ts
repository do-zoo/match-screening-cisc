...
import {
  deleteVenueMenuImage,
  uploadVenueMenuImage,
} from "@/lib/uploads/upload-venue-menu-image";
...
function menuImageFileForRow(
  formData: FormData | null,
  row: VenueCatalogUiPayload["items"][number],
): File | null {
...
async function persistVenueMenuItems(
  ...
  if (!items.length) {
    return rootError(
      "Venue minimal memiliki satu item menu dengan nama valid.",
    );
  }
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const persistedIncomingIds = new Set(
        rowsWithPersistedIds.flatMap((it) =>
      ...
      for (const row of rowsWithPersistedIds) {
        ...
        const hasIncomingImageMetadata =
          row.imageBlobUrl !== undefined || row.imageBlobPath !== undefined;
        ...
        if (typeof row.id === "string" && persistedIncomingIds.has(row.id)) {
          ...
          if (
            (uploaded || row.clearImage) &&
            prev?.imageBlobUrl &&
            prev.imageBlobUrl !== nextImageBlobUrl
          ) {
...
export async function createVenueMinimal(
  ...
  if (!mapUrlParsed.success) {
    const msg =
      mapUrlParsed.error.issues[0]?.message ??
      "Tautan peta harus berupa URL valid (https://…).";
...
export async function saveVenueMenu(
  venueId: string,
  formData: FormData,
): Promise<ActionResult<{ venueId: string }>> {
  try {
  ...
  const parsed = venueMenuOnlyPayloadSchema.safeParse(
    parsePayloadField(formData),
  );
  if (!parsed.success) {
    return fieldError(zodToFieldErrors(parsed.error));
  }
...
export async function saveVenueCatalog(
  ...
  const formData =
    formDataOrPayload instanceof FormData ? formDataOrPayload : null;
  const parsed = venueCatalogPayloadSchema.safeParse(
    parsePayloadField(formDataOrPayload),
  );
  if (!parsed.success) {
    return fieldError(zodToFieldErrors(parsed.error));
  }
