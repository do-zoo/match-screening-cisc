...
const UPLOAD_PURPOSE_BY_KIND: Record<
  EventSettlementArtifactKind,
  UploadPurpose
> = {
  [EventSettlementArtifactKind.venue_transfer]:
    UploadPurpose.event_settlement_venue_transfer,
  [EventSettlementArtifactKind.venue_receipt]:
    UploadPurpose.event_settlement_venue_receipt,
  [EventSettlementArtifactKind.treasurer_margin]:
    UploadPurpose.event_settlement_treasurer_margin,
};
...
export async function uploadEventSettlementProof(
  ...
  const declaredAmountIdr =
    kind === EventSettlementArtifactKind.venue_receipt
      ? null
      : parseIdrDigitsToInt(declaredRaw);

  if (
    kind !== EventSettlementArtifactKind.venue_receipt &&
    (declaredAmountIdr === null || declaredAmountIdr < 0)
  ) {
  ...
  if (kind === EventSettlementArtifactKind.venue_transfer) {
    ...
    const { delta, withinTolerance } = settlementAmountMismatch(
      declared,
      expectedAmountIdr,
    );
    amountDeltaIdr = delta;
    const rawAck =
      String(formData.get("mismatchAcknowledged") ?? "") === "true";
    ...
    if (!withinTolerance) {
      ...
      if (!mismatchAcknowledged || mismatchReason.length < 3) {
        return rootError(
          "Nominal berbeda dari acuan laporan. Centang konfirmasi dan isi alasan (minimal 3 karakter).",
        );
      }
  ...
  try {
    ...
    artifactRow = await prisma.eventSettlementArtifact.create({
      data: {
        ...
        mismatchReason:
          mismatchReason && mismatchReason.length > 0
            ? mismatchReason.slice(0, MISMATCH_REASON_MAX)
            : null,
