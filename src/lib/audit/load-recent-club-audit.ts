...
function parseRangeBound(raw: string | undefined, kind: 'start' | 'end'): Date | undefined {
...
export function buildClubAuditWhere(filters: ClubAuditListFilters): Prisma.ClubAuditLogWhereInput {
...
export const loadClubAuditList = cache(
  async (requestedPage: number, filters: ClubAuditListFilters): Promise<ClubAuditPageResult> => {
    const where = buildClubAuditWhere(filters)
    const totalItems = await prisma.clubAuditLog.count({ where })
    const page = resolveClampedPage(requestedPage, totalItems, CLUB_AUDIT_PAGE_SIZE)
