...
describe("computeBadgeStatus", () => {
  ...
  it("returns 'closed' when now is before openRegistrationAt", () => {
    expect(
      computeBadgeStatus({ ...base, now: new Date("2026-05-31T23:59:59Z") })
    ).toBe("closed");
  });

  it("returns 'closed' when now is at or after closeRegistrationAt", () => {
    expect(
      computeBadgeStatus({ ...base, now: new Date("2026-06-05T00:00:00Z") })
    ).toBe("closed");
    expect(
      computeBadgeStatus({ ...base, now: new Date("2026-06-06T00:00:00Z") })
    ).toBe("closed");
  });
  ...
  it("returns 'closing_soon' when within 12 hours of closeRegistrationAt", () => {
    expect(
      computeBadgeStatus({ ...base, now: new Date("2026-06-04T12:00:00Z") })
    ).toBe("closing_soon");
    expect(
      computeBadgeStatus({ ...base, now: new Date("2026-06-04T23:59:59Z") })
    ).toBe("closing_soon");
  });

  it("returns 'open' outside the closing_soon window", () => {
    expect(
      computeBadgeStatus({ ...base, now: new Date("2026-06-04T11:59:59Z") })
    ).toBe("open");
  });
});
