 ...
 */
function registrationCapacityLimit(registrationCapacity: number | null | undefined): number | null {
...
export type EventRegistrationGatePick = Pick<
  Event,
  'status' | 'registrationManualClosed' | 'registrationCapacity' | 'openRegistrationAt' | 'closeRegistrationAt'
>
...
export async function assertRegistrationAcceptableOrThrowForTx(
  ...
  const registrationsTowardQuota = await countRegistrationsTowardQuota(db, event.id)
