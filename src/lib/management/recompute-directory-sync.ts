...
import { computeIsManagementMemberForMember, type BoardAssignmentRow } from '@/lib/management/recompute-directory-flags'
...
export async function recomputeAllLinkedDirectoryFlagsTx(tx: Prisma.TransactionClient): Promise<void> {
  const links = await tx.managementMember.findMany({
    where: { masterMemberId: { not: null } },
    select: { masterMemberId: true },
  })
  const ids = links.map(l => l.masterMemberId).filter((id): id is string => id !== null)
