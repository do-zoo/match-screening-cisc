...
function assignmentSearchClause(q?: string): Prisma.BoardAssignmentWhereInput {
...
export async function countPeriodAssignmentsByTabForAdmin(opts: {
  ...
  const [all, linked, unlinked] = await Promise.all([
    ...
    prisma.boardAssignment.count({
      where: {
        AND: [base, { managementMember: { masterMemberId: { not: null } } }, ...(hasSearch ? [searchOnly] : [])],
      },
    }),
    prisma.boardAssignment.count({
      where: {
        AND: [base, { managementMember: { masterMemberId: null } }, ...(hasSearch ? [searchOnly] : [])],
      },
    }),
  ])
