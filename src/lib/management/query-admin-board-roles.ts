...
function boardRoleSearchOnlyWhere(q?: string): Prisma.BoardRoleWhereInput {
...
export function boardRoleAdminWhere(opts: { filter: BoardRoleAdminFilter; q?: string }): Prisma.BoardRoleWhereInput {
  const searchClause = boardRoleSearchOnlyWhere(opts.q)
  const activityClause: Prisma.BoardRoleWhereInput =
    opts.filter === 'active' ? { isActive: true } : opts.filter === 'inactive' ? { isActive: false } : {}
...
export async function countBoardRolesByTabForAdmin(opts: {
  ...
  const [all, active, inactive] = await Promise.all([
    ...
    prisma.boardRole.count({
      where: hasSearch ? { AND: [searchOnly, { isActive: true }] } : { isActive: true },
    }),
    prisma.boardRole.count({
      where: hasSearch ? { AND: [searchOnly, { isActive: false }] } : { isActive: false },
    }),
  ])
...
export async function countBoardRolesForAdmin(opts: { filter: BoardRoleAdminFilter; q?: string }): Promise<number> {
