...
function managementMemberSearchOnlyWhere(q?: string): Prisma.ManagementMemberWhereInput {
...
export async function countManagementMembersByTabForAdmin(opts: {
  ...
  const [all, linked, unlinked] = await Promise.all([
    ...
    prisma.managementMember.count({
      where: hasSearch ? { AND: [searchOnly, { masterMemberId: { not: null } }] } : { masterMemberId: { not: null } },
    }),
    prisma.managementMember.count({
      where: hasSearch ? { AND: [searchOnly, { masterMemberId: null }] } : { masterMemberId: null },
    }),
  ])
