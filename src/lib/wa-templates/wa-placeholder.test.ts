...
describe('applyWaPlaceholders', () => {
  ...
  it('throws on unknown token lookup', () => {
    expect(() => applyWaPlaceholders('Halo {nama}.', {})).toThrow(/nilai hilang/i)
  })

  it('throws on stray braces after replace', () => {
    expect(() => applyWaPlaceholders('Invalid {nama', { nama: 'x' })).toThrow(/Template berisi|Tidak dikenali|kurung/)
  })
})
