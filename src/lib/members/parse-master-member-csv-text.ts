...
export function parseMasterMemberCsvText(csvText: string): {
  ...
  const fieldsHeader = parsed.meta.fields?.map(normalizeHeader).filter(Boolean) ?? []
  const hasManagementMemberColumn =
    fieldsHeader.includes('is_management_member') || fieldsHeader.includes(LEGACY_IS_MANAGEMENT_MEMBER_HEADER)
  ...
  for (const col of MASTER_MEMBER_CSV_COLUMNS) {
    if (col === 'is_management_member' || col === 'email' || col === 'whatsapp') continue
  ...
  const rows: ParsedMasterMemberCsvRow[] = parsed.data.map((record, i) => {
    ...
    for (const [k, v] of Object.entries(record)) {
      ...
      if (nk === LEGACY_IS_MANAGEMENT_MEMBER_HEADER) {
        legacyManagementMember = typeof v === 'string' ? v : String(v ?? '')
        continue
      }
    ...
    if (clean.is_management_member === undefined || clean.is_management_member === '') {
