...
describe('build + sort + filter + recap', () => {
  ...
  it('tab active excludes draft/finished', () => {
    const items = sortAdminEventSummaries(buildAdminEventSummaries(rows, pending, approved, total))
  ...
  it('recap sums pending only on visible tab rows', () => {
    ...
    expect(sumPendingReviewForSummaries(filterAdminEventSummariesByTab(items, 'all'))).toBe(12)
    expect(sumPendingReviewForSummaries(filterAdminEventSummariesByTab(items, 'draft'))).toBe(7)
  })
})
