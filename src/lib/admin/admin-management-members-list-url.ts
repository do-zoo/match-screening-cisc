import { parseEventsIndexSearchQuery } from '@/lib/admin/events-index-view'
import type { ManagementMemberAdminFilter } from '@/lib/management/query-admin-management-members'
import { parseAdminTablePage } from '@/lib/table/admin-pagination'

export function parseManagementMemberAdminFilter(
  raw: string | string[] | undefined,
): ManagementMemberAdminFilter {
  const v = Array.isArray(raw) ? raw[0] : raw
  if (v === 'linked' || v === 'unlinked') return v
  return 'all'
}

export function parseAdminManagementMembersListSearchParams(
  sp: Record<string, string | string[] | undefined>,
): {
  filter: ManagementMemberAdminFilter
  q: string
  page: number
} {
  return {
    filter: parseManagementMemberAdminFilter(sp.filter),
    q: parseEventsIndexSearchQuery(sp.q),
    page: parseAdminTablePage(sp.page),
  }
}

/** URL daftar pengurus (`?filter=`, `?q=`, `?page=`). */
export function buildAdminManagementMembersListUrl(opts: {
  filter: ManagementMemberAdminFilter
  q?: string
  page?: number
}): string {
  const p = new URLSearchParams()
  if (opts.filter !== 'all') p.set('filter', opts.filter)
  const qq = opts.q?.trim()
  if (qq) p.set('q', qq.slice(0, 200))
  if (opts.page !== undefined && opts.page > 1) p.set('page', String(opts.page))
  const s = p.toString()
  return s ? `/admin/management/members?${s}` : '/admin/management/members'
}

export function adminManagementMembersListPreservedQuery(opts: {
  filter: ManagementMemberAdminFilter
  q: string
}): Record<string, string | undefined> {
  const out: Record<string, string | undefined> = {}
  if (opts.filter !== 'all') out.filter = opts.filter
  const term = opts.q.trim()
  if (term) out.q = term
  return out
}
