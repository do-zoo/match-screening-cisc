...
export function parseEventsIndexStatusTab(raw: string | string[] | undefined): EventsIndexStatusTab {
...
export function groupByResultToCountMap(rows: { eventId: string; count: number }[]): Record<string, number> {
...
export function sortAdminEventSummaries(items: AdminEventSummary[]): AdminEventSummary[] {
...
export function filterAdminEventSummariesBySearch(items: AdminEventSummary[], raw: string): AdminEventSummary[] {
...
export function sumPendingReviewForSummaries(items: AdminEventSummary[]): number {
