...
describe('findDuplicateMemberNumbers', () => {
  ...
  it('returns duplicates when DB has same member on event', async () => {
    vi.mocked(prisma.registration.findMany).mockResolvedValueOnce([{ claimedMemberNumber: '123' } as never])
  ...
  it('returns only duplicate member numbers from a mixed list', async () => {
    vi.mocked(prisma.registration.findMany).mockResolvedValueOnce([{ claimedMemberNumber: '456' } as never])
