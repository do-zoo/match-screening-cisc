...
const BLOB_PATH = 'site/logo.webp'

/** Deterministic public club logo URL (overwrite). Deletes previous Blob best-effort. */
export async function uploadClubLogo(opts: {
  ...
  const webp = await toWebp(raw, { maxDim: 480, quality: 86 })

  const putRes = await retry(() => putWebpToBlob({ path: BLOB_PATH, bytes: webp.bytes }), {
    maxAttempts: 3,
    delayMs: 250,
  })
