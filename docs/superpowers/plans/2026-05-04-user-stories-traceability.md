...
pnpm vitest run src/lib/forms/submit-registration-schema.test.ts \
  ...
  src/lib/events/registration-window.test.ts
```
...
pnpm vitest run src/lib/actions/admin-events.test.ts \
  ...
  src/tests/unit/tickets-eventid.test.ts \
  src/tests/unit/permissions.test.ts
```

Paket di atas menutup subset kode (event/editor, inbox path, voucher, format error, permissions, tiket). Modul action reviewer lain (`verify-registration`, `attendance`, `invoice-adjustment`, `member-validation`, dll.) belum punya file `*.test.ts` khusus; andalkan QA manual untuk alur menyeluruh setelah regressi paket ini lulus.
