...
export function applyWaPlaceholders(template: string, vars: Record<string, string>): string {
...
describe('applyWaPlaceholders', () => {
  ...
  it('throws on unknown token lookup', () => {
    expect(() => applyWaPlaceholders('Halo {nama}.', {})).toThrow(/Missing value.*nama/)
  })

  it('throws on stray braces after replace', () => {
    expect(() => applyWaPlaceholders('Invalid {nama', { nama: 'x' })).toThrow(/tidak dikenal|seimbang/)
  })
})
```
...
import { REQUIRED_TOKENS, validateWaTemplateBody } from '@/lib/wa-templates/wa-template-policy'
...
Setiap konstanta string di bawah memakai **token snake_case sesuai** `REQUIRED_TOKENS`; format whitespace dan tanda **_WhatsApp‑markdown_** meniru perilaku **`messages.ts`**.
...
describe('CLUB_WA_DEFAULT_BODIES validity', () => {
  ...
  for (const key of keys) {
    it(`defaults valid for ${key}`, () => {
      expect(validateWaTemplateBody(key, CLUB_WA_DEFAULT_BODIES[key])).toBeNull()
    })
  }
})
```
...
export function renderApprovedMessage(
  ...
  if (!bodyFromDb) return templateApproved(eventTitle, venue, startAtIso)
...
export async function resetClubWaTemplateBody(
  ...
  const parsedKey = saveClubWaTemplateSchema.shape.key.safeParse(formData.get('key'))
...
export default async function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className='flex min-h-full flex-1 flex-col'>
      <PublicHeader clubNameNav={branding.clubNameNav} logoUrl={branding.logoBlobUrl} />
      {children}
