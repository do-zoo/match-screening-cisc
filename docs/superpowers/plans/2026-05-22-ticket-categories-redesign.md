...
type DialogState = { mode: 'closed' } | { mode: 'create' } | { mode: 'edit'; category: EventTicketCategoryRow }
...
export function TicketCategoriesPanel({ eventId, categories: initial }: Props) {
  ...
  async function handleToggle(id: string, isActive: boolean) {
    startTransition(async () => {
      ...
      if (!res.ok) {
        toastActionErr(res, 'Gagal mengubah status.')
        return
      }
              ...
              {categories.map(cat => (
                <tr key={cat.id} className='border-t'>
                  <td className='px-3 py-2'>
                    {cat.name}
                    {!cat.isActive && <span className='ml-2 text-xs text-muted-foreground'>(nonaktif)</span>}
                  </td>
                  ...
                  <td className='px-3 py-2'>
                    <div className='flex justify-end gap-1'>
                      <Button size='sm' variant='ghost' onClick={() => setDialogState({ mode: 'edit', category: cat })}>
                        Edit
                      </Button>
                      <Button
                        size='sm'
  ...
  function onSubmit(values: TicketCategoryInput) {
    startTransition(async () => {
      if (mode === 'create') {
        ...
        if (!res.ok) {
          toastActionErr(res, 'Gagal menyimpan.')
          return
        }
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <IdrAmountInput value={field.value} onChange={field.onChange} disabled={priceLocked} />
                    </FormControl>
                    {priceLocked && (
                      <p className='text-xs text-muted-foreground'>Harga tidak dapat diubah — sudah ada registrasi.</p>
                    )}
                    <FormMessage />
                  </FormItem>
                )}
...
export function HolderCard({ index, isPrimary, menuItems, menuRequired }: Props) {
  ...
  const summary = holderName ? `${holderName}${memberNumber ? ` · ${memberNumber}` : ''}` : 'Belum diisi'
        ...
        <div className='space-y-3'>
          ...
          <FormItem>
            ...
            {form.formState.errors.transferProof && (
              <p className='text-sm text-destructive'>{form.formState.errors.transferProof.message}</p>
            )}
          </FormItem>
        </div>
...
export async function submitRegistration(
  ...
  try {
    ...
    const upload = await uploadImageForRegistration(tempId, 'transfer_proof', input.transferProof as File)
