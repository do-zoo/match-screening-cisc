...
export type NormalizeDisplayNameResult = { ok: true; value: string } | { ok: false; message: string }
...
export async function updateAdminDisplayName(formData: FormData): Promise<ActionResult<void>> {
...
export function AdminAccountPageClient({ initialName, email }: { initialName: string; email: string }) {
...
export default async function AdminAccountPage() {
  ...
  return (
    <main className='flex flex-1 flex-col'>
      <AdminAccountPageClient initialName={session.user.name ?? ''} email={session.user.email ?? ''} />
    </main>
  )
}
```
...
export function AdminAccountMenu({ userEmail, displayName, triggerClassName }: AdminAccountMenuProps) {
...
```tsx
return (
  <AdminAppShell navFlags={navFlags} userEmail={session.user.email ?? null} displayName={session.user.name ?? null}>
    {children}
  </AdminAppShell>
)
```
