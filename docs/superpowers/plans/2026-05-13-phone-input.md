    ...
    <form onSubmit={form.handleSubmit(console.log)}>
      <Controller
        ...
        render={({ field, fieldState }) => (
          <PhoneInput value={field.value} onChange={field.onChange} aria-invalid={Boolean(fieldState.error)} />
        )}
      />
      {form.formState.errors.phone?.nationalNumber?.message && (
        <p className='text-sm text-destructive'>{form.formState.errors.phone.nationalNumber.message}</p>
      )}
    </form>
  )
}
