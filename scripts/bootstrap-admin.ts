...
async function main() {
  const [{ auth }, { prisma }] = await Promise.all([import('@/lib/auth/auth'), import('@/lib/db/prisma')])
  ...
  const role = (readArg('--role') ?? process.env.BOOTSTRAP_ADMIN_ROLE ?? 'Owner') as Args['role']
  ...
  const { appendClubAuditLog } = await import('@/lib/audit/append-club-audit-log')
