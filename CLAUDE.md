...
- **`ClubOperationalSettings`** / **`ClubNotificationPreferences`** — singleton (`singletonKey = "default"`); prefs: `outboundMode` + auto-email (`emailAutoOnApprove` … `emailAutoOnRefund`, default hanya approve) + `emailAttachInvoicePdf` (default `true`, lampiran PDF pada email tagihan); read via `lib/public/load-club-*.ts`; mutations Owner-only + audit
...
- `lib/invoices/registration-invoice-pdf-doc.tsx` — komponen `@react-pdf/renderer` `RegistrationInvoicePdfDocument`
...
- `lib/invoices/try-build-invoice-email-attachment.ts` — `tryBuildInvoiceEmailAttachment` (PDF lampiran email invoice/underpayment; gagal render → null, tidak memblokir kirim)
