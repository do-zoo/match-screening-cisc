...
- `admin/` — authenticated admin area (all routes require a session; redirect enforced in `src/proxy.ts` + `admin/layout.tsx`)
  ...
  - `admin/events/[eventId]/registrants` — daftar peserta (toolbar: `?q=`, `?tab=` status pendaftaran, `?view=tabel` vs kartu, `?page=`); `tab` kosong/invalid diperlakukan sebagai `pending_review` (tanpa redirect URL) agar layar awal fokus antrian verifikasi; label nav **Peserta Acara**; URL lama `/admin/events/[eventId]/inbox` dialihkan permanen lewat `next.config.ts`
...
- `api/admin/events/[eventId]/registrants/[registrationId]/invoice-pdf` — GET PDF tagihan (`?kind=registration|adjustment`, `?adjustmentId=`, `?disposition=inline|attachment`); `guardEvent`
...
- `lib/actions/admin-venues.ts` — `saveVenueBasics` / `saveVenueMenu` / `saveVenueCatalog` (payload penuh, dipakai uji); `deleteVenue` (Owner/Admin; tolak bila venue punya acara; redirect indeks + flash); unggah gambar menu venue; item terkunci pendaftar mengikuti `venueMenuItemIdsFrozenByExistingRegistrations`
...
- `lib/invoices/registration-invoice-pdf-types.ts` — `RegistrationInvoicePdfVm`, `InvoicePdfKind`, `InvoicePdfPaymentStatus`
- `lib/invoices/registration-invoice-pdf-eligibility.ts` — `canDownloadRegistrationInvoicePdf` (status terminal menolak unduhan tagihan pendaftaran)
- `lib/invoices/registration-invoice-pdf-filename.ts` — `buildInvoicePdfFilename` (`tagihan-{slug}-{id8}.pdf` / `penyesuaian-…`)
- `lib/invoices/registration-invoice-pdf-data.ts` — `loadRegistrationInvoicePdfData` (Prisma + branding + line items)
- `lib/invoices/registration-invoice-pdf-doc.tsx` — komponen `@react-pdf/renderer` `RegistrationInvoicePdfDocument`; layout memakai `EMAIL_DESIGN_TOKENS` (header/footer biru, kartu ringkasan, badge status)
- `lib/invoices/resolve-invoice-pdf-logo-src.ts` — konversi logo WebP (Blob atau fallback `public/logo.webp`) ke data URI PNG untuk react-pdf
- `lib/invoices/render-registration-invoice-pdf.ts` — `renderRegistrationInvoicePdf(vm)` → `{ buffer, filename, contentType }`
- `lib/invoices/build-registration-invoice-pdf-url.ts` — `buildRegistrationInvoicePdfUrl` (path + query untuk pratinjau/unduh admin)
- `lib/invoices/try-build-invoice-email-attachment.ts` — `REGISTRATION_EMAIL_PDF_ATTACHMENT_KEYS` + `tryBuildInvoiceEmailAttachment` (PDF lampiran email `invoice` / `invoice_underpayment` / `receipt` / `registration_approved`; gagal render → null, tidak memblokir kirim)
