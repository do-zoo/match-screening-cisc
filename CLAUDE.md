...
- **`MasterMember`** — the club member directory; `isManagementMember` is **derived from kepengurusan** (`BoardAssignment` for the active `BoardPeriod` when the pengurus row links via `ManagementMember.masterMemberId`), not edited manually in the directory UI
- **`Event`** — slug, timeline **`openRegistrationAt` / `closeRegistrationAt` / `openGateAt` / `kickOffAt`**, **`mandatoryMenuItemIds`** (subset of the event’s **`EventVenueMenuItem`** rows), linked menu via **`EventVenueMenuItem`**; `multiCategoryPurchase Boolean` — izinkan beli lintas kategori; `ticketCategories EventTicketCategory[]` — kategori tiket per acara; financial PIC is **`picAdminProfileId`** (`AdminProfile`); pembayaran via **`bankAccountId`** → **`PicBankAccount`**
...
- **`Registration`** — **satu baris per transaksi** (bukan per tiket); `ticketCategoryId` → `EventTicketCategory`; `ticketQty` = total tiket; `computedTotalAtSubmit` = sum `ticketPriceApplied` semua holder; konsep partner **dihapus**; status flows: `submitted → pending_review → approved / rejected / payment_issue`
- **`RegistrationHolder`** — satu baris per tiket dalam transaksi; `sortOrder=1` = pemesan utama; `holderName`, `holderWhatsapp` (opsional — diisi dari direktori jika member terverifikasi, atau manual oleh pendaftar), `claimedMemberNumber`, `memberValidation` (`MemberValidation` enum), `ticketPriceApplied` (snapshot), `mandatoryMenuItemId` (nullable jika menu tidak wajib)
- **`EventTicketCategory`** — kategori tiket per acara: `name`, `regularPrice`, `memberPrice` (IDR whole rupiah), `maxQtyPerPerson` (null=tak terbatas), `isActive`, `sortOrder`; admin dapat tambah/edit/hapus lewat tab Harga & Tiket; harga terkunci setelah ada registrasi pertama
...
- `lib/reports/csv.ts` — `generateRegistrationsCsv(eventId)` — CSV UTF-8 satu baris per `Registration`; kolom holder dinamis (Holder N Nama · Member · Status · Harga) sesuai `ticketQty` maksimal; plus kolom kategori, status, penyesuaian
...
- `lib/registrations/admin-ticket-context.ts` — `RegistrationHolderContext` type + `aggregateCrossRegistrationConflicts`; builds holder context for admin registration detail page
